...
func (c *UeditorController) ControllerUE() {
	...
	case "uploadimage", "uploadfile", "uploadvideo":
		...
		err := os.MkdirAll("/home/wuchao/mygo/src/golangblog/static/upload/"+number+name, 0777) //..代表本当前exe文件目录的上级，.表示当前目录，没有.表示盘的根目录
		if err != nil {
			beego.Error(err)
		}
		//保存上传的图片
		...
		path1 := "/home/wuchao/mygo/src/golangblog/static/upload/" + number + name + "/" + h.Filename
		err = c.SaveToFile("upfile", path1) //.Join("attachment", attachment)) //存文件    WaterMark(path)    //给文件加水印
		if err != nil {
			beego.Error(err)
		}
		c.Data["json"] = map[string]interface{}{"state": "SUCCESS", "url": "/static/upload/" + number + name + "/" + h.Filename, "title": h.Filename, "original": h.Filename}
		c.ServeJSON()
		// 		{
		//     "state": "SUCCESS",
		//     "url": "upload/demo.jpg",
